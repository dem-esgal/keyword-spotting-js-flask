...
function predictKeyword(x, model, commands) {
  ...
  let max_prob = 0;
  // const weights = [1000, 10, 10, 10, 0, 10, 10, 10, 5, 0, 10, 10];
  const weights = [1, 1, 0, 0, 0, 1, 1, 1, 0, 0, 1, 1];

  let sum = 0;
  const noise_level = (output[0] + output[1] + 0.05) - Math.min(output[0], output[1], 0.05) - Math.max(output[0], output[1], 0.05) ;
  for (let i = 0; i < commands.length; i++) {
    output[i] = (output[i] + noise_level) * weights[i];
    sum += output[i];
  }
  ...
  if (commands[index] !== 'silence' && commands[index] !== 'unknown') {
    console.log('Command: ' + commands[index]);
    console.log(JSON.stringify(model.predict(x)))
  }
